...
const Services = () => {
    ...
    const filterServices = (service) => {
        ...
        if (profession && service.profession.nameProfession !== profession) {
            return false;
        }

        if (provincia && service.province.nameProvince !== selectProv) {
            return false;
        }

        if (selectedCityId && service.city.nameCity !== selectedCityId) {
            return false;
        }
    ...
    return (
        <section className='lg:p-10 flex flex-col justify-center items-center'>
            ...
            <div className='md:h-[150px] text-xs p-5 flex md:flex-row flex-col items-center md:justify-between lg:max-w-[1100px] lg:w-full'>
                {services.length > 0 ? (<div>{services.length} Profesionales disponibles</div>) : <div>No hay profesionales disponibles</div>}
                                ...
                                <Cards className='flex flex-col items-center'>
                                    ...
                                    <img src={item.imageUrl ? "http://localhost:8080/user/images/" + item.imageUrl : "default-image.jpg"} className=' rounded-full w-1/2 mb-4 cover' />
                                        <div className=''>
                                            ...
                                            <div>{item.city.nameCity},</div>
                                            <div> {item.province.nameProvince}</div>
                                        </div>
                                    </div>
